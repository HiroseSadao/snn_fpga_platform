...
void _before_run_inp_exc_pre_push_spikes()
{
    ...
    ///// CONSTANTS ///////////
    const int64_t _n_sources = 784;
const int64_t _n_targets = 50;
const size_t _num_source_dt = 1;
const size_t _num_spikespace = 785;
double* const _array_inp_exc_delay = _dynamic_array_inp_exc_delay.empty()? 0 : &_dynamic_array_inp_exc_delay[0];
const size_t _numdelay = _dynamic_array_inp_exc_delay.size();
    ///// POINTERS ////////////
        
    double*   _ptr_array_defaultclock_dt = _array_defaultclock_dt;
    int32_t* __restrict  _ptr_array_inp__spikespace = _array_inp__spikespace;
    double* __restrict  _ptr_array_inp_exc_delay = _array_inp_exc_delay;

    std::vector<double> &real_delays = _dynamic_array_inp_exc_delay;
    double* real_delays_data = real_delays.empty() ? 0 : &(real_delays[0]);
    int32_t* sources = inp_exc_pre.sources.empty() ? 0 : &(inp_exc_pre.sources[0]);
    const size_t n_delays = real_delays.size();
    const size_t n_synapses = inp_exc_pre.sources.size();
    inp_exc_pre.prepare(_n_sources,
                           _n_targets,
