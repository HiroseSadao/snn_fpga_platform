...
void _run_inh_spike_thresholder_codeobject()
{
...
const double v_thresh = - 0.04;
    ///// POINTERS ////////////
        
    int32_t* __restrict  _ptr_array_inh__spikespace = _array_inh__spikespace;
    double* __restrict  _ptr_array_inh_lastspike = _array_inh_lastspike;
    char* __restrict  _ptr_array_inh_not_refractory = _array_inh_not_refractory;
    double*   _ptr_array_defaultclock_t = _array_defaultclock_t;
    double* __restrict  _ptr_array_inh_v = _array_inh_v;
    ...
    for(size_t _idx=0; _idx<N; _idx++)
    {
        ...
        const char not_refractory = _ptr_array_inh_not_refractory[_idx];
        const double v = _ptr_array_inh_v[_idx];
        char _cond;
        if(!not_refractory)
            _cond = (v > v_thresh) && false;
        else 
            _cond = (v > v_thresh) && true;

        if(_cond) {
            _ptr_array_inh__spikespace[_count++] = _idx;
            _ptr_array_inh_not_refractory[_idx] = false;
            _ptr_array_inh_lastspike[_idx] = _ptr_array_defaultclock_t[0];
        }
    }
    _ptr_array_inh__spikespace[N] = _count;
