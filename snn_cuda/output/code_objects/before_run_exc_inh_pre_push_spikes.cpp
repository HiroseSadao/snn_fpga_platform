...
const int64_t _n_targets = 50;
const size_t _num_source_dt = 1;
const size_t _num_spikespace = 51;
double* const _array_exc_inh_delay = _dynamic_array_exc_inh_delay.empty()? 0 : &_dynamic_array_exc_inh_delay[0];
const size_t _numdelay = _dynamic_array_exc_inh_delay.size();
    ///// POINTERS ////////////
        
    double*   _ptr_array_defaultclock_dt = _array_defaultclock_dt;
    int32_t* __restrict  _ptr_array_exc__spikespace = _array_exc__spikespace;
    double* __restrict  _ptr_array_exc_inh_delay = _array_exc_inh_delay;

    std::vector<double> &real_delays = _dynamic_array_exc_inh_delay;
    double* real_delays_data = real_delays.empty() ? 0 : &(real_delays[0]);
    int32_t* sources = exc_inh_pre.sources.empty() ? 0 : &(exc_inh_pre.sources[0]);
    const size_t n_delays = real_delays.size();
    const size_t n_synapses = exc_inh_pre.sources.size();
    exc_inh_pre.prepare(_n_sources,
                           _n_targets,
