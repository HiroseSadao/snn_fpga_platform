...
double* const _array_inh_exc_delay = _dynamic_array_inh_exc_delay.empty()? 0 : &_dynamic_array_inh_exc_delay[0];
const size_t _numdelay = _dynamic_array_inh_exc_delay.size();
    ///// POINTERS ////////////
        
    double*   _ptr_array_defaultclock_dt = _array_defaultclock_dt;
    int32_t* __restrict  _ptr_array_inh__spikespace = _array_inh__spikespace;
    double* __restrict  _ptr_array_inh_exc_delay = _array_inh_exc_delay;

    std::vector<double> &real_delays = _dynamic_array_inh_exc_delay;
    double* real_delays_data = real_delays.empty() ? 0 : &(real_delays[0]);
    int32_t* sources = inh_exc_pre.sources.empty() ? 0 : &(inh_exc_pre.sources[0]);
    const size_t n_delays = real_delays.size();
    const size_t n_synapses = inh_exc_pre.sources.size();
    inh_exc_pre.prepare(_n_sources,
                           _n_targets,
