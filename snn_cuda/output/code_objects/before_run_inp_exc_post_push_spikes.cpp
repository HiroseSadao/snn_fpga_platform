...
void _before_run_inp_exc_post_push_spikes()
{
    ...
    ///// CONSTANTS ///////////
    const int64_t _n_sources = 50;
const int64_t _n_targets = 784;
const size_t _num_source_dt = 1;
const size_t _num_spikespace = 51;
double* const _array_inp_exc_delay_1 = _dynamic_array_inp_exc_delay_1.empty()? 0 : &_dynamic_array_inp_exc_delay_1[0];
const size_t _numdelay = _dynamic_array_inp_exc_delay_1.size();
    ///// POINTERS ////////////
        
    double*   _ptr_array_defaultclock_dt = _array_defaultclock_dt;
    int32_t* __restrict  _ptr_array_exc__spikespace = _array_exc__spikespace;
    double* __restrict  _ptr_array_inp_exc_delay_1 = _array_inp_exc_delay_1;

    std::vector<double> &real_delays = _dynamic_array_inp_exc_delay_1;
    double* real_delays_data = real_delays.empty() ? 0 : &(real_delays[0]);
    int32_t* sources = inp_exc_post.sources.empty() ? 0 : &(inp_exc_post.sources[0]);
    const size_t n_delays = real_delays.size();
    const size_t n_synapses = inp_exc_post.sources.size();
    inp_exc_post.prepare(_n_sources,
                           _n_targets,
