...
void _run_exc_inh_synapses_create_generator_codeobject()
{
...
int32_t* const _array_exc_inh_N_incoming = _dynamic_array_exc_inh_N_incoming.empty()? 0 : &_dynamic_array_exc_inh_N_incoming[0];
const size_t _numN_incoming = _dynamic_array_exc_inh_N_incoming.size();
int32_t* const _array_exc_inh_N_outgoing = _dynamic_array_exc_inh_N_outgoing.empty()? 0 : &_dynamic_array_exc_inh_N_outgoing[0];
const size_t _numN_outgoing = _dynamic_array_exc_inh_N_outgoing.size();
const int64_t N_post = 50;
const int64_t N_pre = 50;
const int64_t _source_offset = 0;
int32_t* const _array_exc_inh__synaptic_post = _dynamic_array_exc_inh__synaptic_post.empty()? 0 : &_dynamic_array_exc_inh__synaptic_post[0];
const size_t _num_synaptic_post = _dynamic_array_exc_inh__synaptic_post.size();
int32_t* const _array_exc_inh__synaptic_pre = _dynamic_array_exc_inh__synaptic_pre.empty()? 0 : &_dynamic_array_exc_inh__synaptic_pre[0];
const size_t _num_synaptic_pre = _dynamic_array_exc_inh__synaptic_pre.size();
const int64_t _target_offset = 0;
    ///// POINTERS ////////////
        
    int32_t*   _ptr_array_exc_inh_N = _array_exc_inh_N;
    int32_t* __restrict  _ptr_array_exc_inh_N_incoming = _array_exc_inh_N_incoming;
    int32_t* __restrict  _ptr_array_exc_inh_N_outgoing = _array_exc_inh_N_outgoing;
    int32_t* __restrict  _ptr_array_exc_inh__synaptic_post = _array_exc_inh__synaptic_post;
    int32_t* __restrict  _ptr_array_exc_inh__synaptic_pre = _array_exc_inh__synaptic_pre;


    const size_t _N_pre = N_pre;
    const size_t _N_post = N_post;
    _dynamic_array_exc_inh_N_incoming.resize(_N_post + _target_offset);
    _dynamic_array_exc_inh_N_outgoing.resize(_N_pre + _source_offset);
    ...
    for(size_t _i=0; _i<_N_pre; _i++)
    {
        ...
        for(long _=_uiter_low; _<_uiter_high; _+=_uiter_step)
        {
            ...
            for (size_t _repetition=0; _repetition<_n; _repetition++) {
                _dynamic_array_exc_inh_N_outgoing[_pre_idx] += 1;
                _dynamic_array_exc_inh_N_incoming[_post_idx] += 1;
                _dynamic_array_exc_inh__synaptic_pre.push_back(_pre_idx);
                _dynamic_array_exc_inh__synaptic_post.push_back(_post_idx);
			}
		}
	}

	// now we need to resize all registered variables
	const int32_t newsize = _dynamic_array_exc_inh__synaptic_pre.size();
    _dynamic_array_exc_inh__synaptic_post.resize(newsize);
    _dynamic_array_exc_inh__synaptic_pre.resize(newsize);
    _dynamic_array_exc_inh_delay.resize(newsize);
	// Also update the total number of synapses
	_ptr_array_exc_inh_N[0] = newsize;
