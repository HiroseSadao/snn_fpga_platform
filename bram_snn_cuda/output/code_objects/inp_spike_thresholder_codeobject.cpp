...
void _run_inp_spike_thresholder_codeobject()
{
...
const size_t _numrates = 784;
    ///// POINTERS ////////////
        
    int32_t* __restrict  _ptr_array_inp__spikespace = _array_inp__spikespace;
    double*   _ptr_array_defaultclock_dt = _array_defaultclock_dt;
    double* __restrict  _ptr_array_inp_rates = _array_inp_rates;
    ...
    for(size_t _idx=0; _idx<N; _idx++)
    {
        ...
        const double rates = _ptr_array_inp_rates[_idx];
        const char _cond = _rand(_vectorisation_idx) < (dt * rates);

        if(_cond) {
            _ptr_array_inp__spikespace[_count++] = _idx;
        }
    }
    _ptr_array_inp__spikespace[N] = _count;
