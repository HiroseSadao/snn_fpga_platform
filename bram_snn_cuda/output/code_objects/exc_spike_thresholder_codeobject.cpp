...
void _run_exc_spike_thresholder_codeobject()
{
...
const double v_thresh_base = - 0.052000000000000005;
    ///// POINTERS ////////////
    ...
    for(size_t _idx=0; _idx<N; _idx++)
    {
        ...
        const char not_refractory = _ptr_array_exc_not_refractory[_idx];
        const double theta = _ptr_array_exc_theta[_idx];
        const double v = _ptr_array_exc_v[_idx];
        char _cond;
        if(!not_refractory)
            _cond = (v > (v_thresh_base + theta)) && false;
        else 
            _cond = (v > (v_thresh_base + theta)) && true;

        if(_cond) {
            _ptr_array_exc__spikespace[_count++] = _idx;
            _ptr_array_exc_not_refractory[_idx] = false;
            _ptr_array_exc_lastspike[_idx] = _ptr_array_defaultclock_t[0];
        }
    }
    _ptr_array_exc__spikespace[N] = _count;
